...
`warehouse/` is a dbt project with local DuckDB and environment-configured BigQuery targets. Four staging views expose features, outcomes, scores and payment exposures. `mart_risk_cohorts` joins one feature and outcome per application, grouped by split, currency and original approval. Source tables are expected in `risk_raw`; quarantine is retained as a raw JSON record table locally.
